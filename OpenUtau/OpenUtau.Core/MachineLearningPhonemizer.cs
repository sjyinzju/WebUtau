...
namespace OpenUtau.Core {
    public abstract class MachineLearningPhonemizer : Phonemizer
    {
        ...
        public override void SetUp(Note[][] groups, UProject project, UTrack track) {
            Log.Information($"MachineLearningPhonemizer.SetUp: groups.Length={groups.Length}");
            if (groups.Length == 0) {
                return;
            }
            ...
            for (int i = 1; i < groups.Length; ++i) {
                ...
                if (groups[i - 1][^1].position + groups[i - 1][^1].duration == groups[i][0].position) {
                    ...
                    try{
                        ...
                        Log.Error(e, $"Failed to phonemize sentence {sentenceLyrics}");
                    }
                    phrase.Clear();
                    phrase.Add(groups[i]);
                }
            ...
            if (phrase.Count > 0) {
                try {
                    ...
                    var sentenceLyrics = string.Join(" ", phrase.Select(group => group[0].lyric));
                    Log.Error(e, $"Failed to phonemize last sentence {sentenceLyrics}");
                }
            }
            Log.Information($"MachineLearningPhonemizer.SetUp: processed {phraseCount} phrases, partResult has {partResult.Count} entries");
        }
